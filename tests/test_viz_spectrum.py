...
def test_empty_judge_steer_raises(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _write(tmp_path, "recovery_demo.csv",
           ["concept", "best_abs_cos", "random_max_abs_cos", "ratio_vs_random",
            "subspace_frac", "subspace_chance"],
           ["demo", "0.30", "0.06", "5.0", "0.40", "0.22"])
    with open(tmp_path / "judge_steer_demo.csv", "w", newline="") as f:      # header only, no rows
        csv.writer(f).writerow(["direction", "scale", "prompt", "completion", "verdict", "reason"])
    with pytest.raises(ValueError):
        build_points(["demo"], present_map={"demo": "FALSE"})
